...
def load_checkpoint(fpath):
    ...
    try:
        ...
        checkpoint = torch.load(fpath, pickle_module=pickle, map_location=map_location, encoding='latin1')
    except Exception:
...
def group_param(model, open_layers):
    group_train = []
    group_hold = []
    if isinstance(model, nn.DataParallel):
        model = model.module
    ...
    for name, module in model.named_children():
        if name in open_layers:
            module.train()
            for p in module.parameters():
                group_train.append(p)
        else:
            module.eval()
            for p in module.parameters():
                group_hold.append(p)
    return group_hold, group_train
